...
export default function Inspector({ panel, onPanelChange, collapsed, onToggleCollapse, floating = false, onClose }: { panel: InspectorPanel; onPanelChange(panel: InspectorPanel): void; collapsed?: boolean; onToggleCollapse?(): void; floating?: boolean; onClose?(): void }) {
  ...
  const motionControls = motionObject && activeScene && <InspectorGroup title="Motion" icon={<Route size={13} />} defaultOpen>
  ...
  return <aside className={`inspector simple-inspector ${floating ? 'floating-inspector' : ''}`}>
    ...
    {panel === 'light' ? <LightingPanel /> : panel === 'scene' ? <div className="scenography-content"><header className="inspector-context scenography-context-header"><strong>{activeScene?.name ?? 'Scene'}</strong><span>Scenography and directions</span></header><InspectorGroup title="Framing and background" icon={<Frame size={13} />} collapsible={false}><BackgroundPanel /></InspectorGroup><ElementsPanel mode="scene" /></div> : <>
      {selectedAudio ? <AudioPanel /> : object && transform ? <section className="object-section edit-stack">
        ...
        <InspectorGroup title="Position and size" icon={<Move3d size={13} />} defaultOpen hint={object.screenSpace ? '2D' : cameraView ? 'Camera' : 'Space'}>
        ...
        <InspectorGroup title="Rotation" icon={<Rotate3d size={13} />}><div className="camera-sliders">
          {([
      ...
      </section> : <div className="camera-edit-panel"><header className="inspector-context"><strong>Camera</strong><span>{activeScene?.name ?? 'Scene'}</span></header>{activeScene && cameraTransform && <InspectorGroup title="Framing" icon={<Frame size={13} />} defaultOpen><div className="camera-sliders"><label><span>Distance</span><strong>{activeScene.framing.distance.toFixed(1)} m</strong><input aria-label="Zoom camera" type="range" min="0.5" max="30" step="0.1" value={Math.min(30, activeScene.framing.distance)} onChange={(event) => setCameraZoom(Number(event.target.value))} /></label>{([
        ['Horizontal', 0, -20, 20], ['Depth', 1, -20, 20], ['Height', 2, -5, 20],
