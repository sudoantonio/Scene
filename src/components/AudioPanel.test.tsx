...
describe('Local audio subtitles', () => {
  it('transcribes locally and saves editable timed captions with the project', async () => {
    ...
    expect(screen.getByRole('checkbox', { name: 'Apply to all' })).toBeChecked();
    fireEvent.click(screen.getByRole('checkbox', { name: 'Apply to all' }));
    ...
    expect(audio.audio.applyCaptionPositionToAll).toBe(false);
  });
});
