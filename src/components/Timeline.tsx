...
export default function Timeline({ collapsed, onToggleCollapse }: { collapsed?: boolean; onToggleCollapse?(): void }) {
  ...
  return <section className={`timeline ${collapsed ? 'collapsed' : ''}`}>
    ...
    <div className="timeline-scroll" onClick={seekFromEmptyTimeline} onWheelCapture={zoomTimelineFromWheel} style={{ '--timeline-content-width': `${Math.round(durationSeconds * timelineZoom)}px`, '--timeline-second-width': `${timelineZoom}px`, '--timeline-thumbnail-width': `${Math.max(timelineZoom, 800 / durationSeconds)}px` } as React.CSSProperties}>
