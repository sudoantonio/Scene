...
export async function planDirection(project: AbacoProject, input: Omit<JevActionInput, 'project'>, run: DecisionRunner): Promise<JevActionPlan> {
  ...
  for (const [groupIndex, group] of groups.entries()) {
    ...
    for (const draft of group) {
      ...
      if (compiled.decision.characterAction && !compiled.blenderPlan.operations.some((operation) => operation.type === 'set_controller_pose')) throw new Error(`No movable control found for the requested body part: “${draft.clause.instruction}”.`);
