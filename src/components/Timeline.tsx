...
import { Check, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Eye, EyeOff, GripVertical, LockKeyhole, Maximize2, MessageSquare, MessageSquarePlus, Minimize2, PanelBottomClose, PanelBottomOpen, Pause, Play, Scissors, Trash2, X } from 'lucide-react';
...
export default function Timeline({ collapsed, onToggleCollapse }: { collapsed?: boolean; onToggleCollapse?(): void }) {
  ...
  const chooseMotion = (mode: Interpolation) => {
    ...
    setTransitionMode(motionDraft.objectId, motionDraft.sceneId, mode);
  ...
  return <section className={`timeline ${fullscreen ? 'fullscreen' : ''} ${collapsed ? 'collapsed' : ''}`}>
    <header className="timeline-toolbar">
      <div className="timeline-context">
        {motionDraft ? <div className="motion-composer"><span>{motionDraft.label}</span>{(['constant', 'bezier', 'linear'] as const).map((mode) => <button key={mode} className={motionDraft.mode === mode ? 'active' : ''} onClick={() => chooseMotion(mode)}>{modeNames[mode]}</button>)}</div> : transitionDraft ? <div className="transition-composer"><span>{transitionDraft.label}</span><input autoFocus placeholder="Descrivi la transizione" value={transitionDraft.text} onChange={(event) => setTransitionDraft({ ...transitionDraft, text: event.target.value })} onKeyDown={(event) => { if (event.key === 'Enter') saveTransition(); if (event.key === 'Escape') setTransitionDraft(undefined); }} /><button disabled={!transitionDraft.text.trim()} onClick={saveTransition}>Salva</button></div> : <strong className="timecode">{timecode}</strong>}
      </div>
    ...
    <div className="timeline-scroll" onWheelCapture={zoomTimelineFromWheel} style={{ '--timeline-content-width': `${Math.round(durationSeconds * timelineZoom)}px`, '--timeline-second-width': `${timelineZoom}px` } as React.CSSProperties}>
      ...
      <div className="track scene-track" onClick={seek}>{scenes.map((scene, index) => {
        ...
        return <div key={scene.id} className={`scene-clip ${activeSceneIndex === index ? 'active' : ''}`} style={{ left: `calc(${left(scene.frame)} + ${insetLeft}px)`, width: `calc(${width}% - ${insetLeft + insetRight}px)` }}>
          <button className={`scene-image ${isSelectedTrack(sceneSelection) ? 'comment-target' : ''}`} style={sceneThumbnails[scene.id] ? { backgroundImage: `linear-gradient(90deg,rgba(20,22,22,.1),rgba(20,22,22,.02)),url(${sceneThumbnails[scene.id]})` } : undefined} title={scene.name ?? `Scena ${index + 1}`} onClick={(event) => { event.stopPropagation(); setFrame(scene.frame); select(undefined); setSelectedTrack(sceneSelection); setCommentDraft(undefined); }}>
          ...
          <button className={`scene-framing ${cameraMode} ${cameraChanged ? 'changed' : 'idle'} ${isSelectedTrack(framingSelection) ? 'comment-target' : ''}`} onClick={(event) => { event.stopPropagation(); setFrame(scene.frame); select(undefined); setSelectedTrack(framingSelection); setCommentDraft(undefined); if (framingCamera && nextScene) setMotionDraft({ objectId: framingCamera.id, sceneId: scene.id, label: `${framingSelection.label} → ${nextScene.name ?? `Scena ${index + 2}`}`, mode: cameraMode }); }}><span>{nextScene ? modeNames[cameraMode] : 'Fissa'}</span>{noteBadge(framingSelection, 'segment-comment')}</button>
      ...
      {timelineObjects.map((object) => { const displayName = object.name; const objectVisible = evaluateProperty(object, 'visibility', frame) as boolean; return <div className={`track-pair object-row ${selectedId === object.id ? 'active' : ''}`} key={object.id}>
        ...
        <div className="track" onClick={seek}>
          {scenes.map((scene, index) => {
            ...
            return <button key={`${object.id}-${scene.id}`} className={`motion-segment ${mode} ${changed ? 'changed' : 'idle'} ${next ? '' : 'static'} ${visible ? '' : 'hidden'} ${isSelectedTrack(selection) ? 'comment-target' : ''}`} style={{ left: left(scene.frame), width: `${width}%` }} title={next ? `${displayName}: ${modeNames[mode]} fra ${scene.name ?? `Scena ${index + 1}`} e ${next.name ?? `Scena ${index + 2}`}` : `${displayName} in ${scene.name ?? `Scena ${index + 1}`}`} onClick={(event) => {
              event.stopPropagation(); select(object.id); setTransitionDraft(undefined); setCommentDraft(undefined); setSelectedTrack(selection);
          ...
          {object.keyframes.filter((key) => !scenes.some((scene) => scene.frame === key.frame)).map((key) => <button key={key.id} className={`key-marker ${key.source}`} style={{ left: left(key.frame) }} title={`${key.property} · frame ${key.frame}`} onClick={(event) => { event.stopPropagation(); setFrame(key.frame); select(object.id); }}>
