...
The application can compose scenes and export projects without an API key. AI integrations are optional: OpenAI powers the advanced Blender plan, while TypeSafe Jev or local Laya converts a bounded character action into typed decisions that Scene compiles into keyframes. API keys are encrypted with Electron's `safeStorage` and are never included in project files. Laya needs no API key; its multilingual model is downloaded to the user's cache on first use and then runs locally.
