...
class RootErrorBoundary extends Component<{ children: ReactNode }, { error?: Error }> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) { console.error('Errore irreversibile nel renderer', error, info); }
  render() {
    ...
    return <main role="alert" style={{ width: '100%', height: '100%', padding: 28, display: 'grid', placeItems: 'center', color: '#e5e5e5', background: '#0d0d0d', fontFamily: 'system-ui, sans-serif' }}>
      <section style={{ width: 'min(480px, 100%)', padding: 22, background: '#202020', border: '1px solid #3b3b3b', borderRadius: 14 }}>
        <h1 style={{ margin: '0 0 8px', fontSize: 18 }}>La vista si è interrotta</h1>
        <p style={{ margin: '0 0 18px', color: '#b8b8b8', fontSize: 13 }}>Il progetto non è stato cancellato. Ricarica il renderer per ripristinare l’interfaccia.</p>
        <button style={{ minHeight: 34, padding: '0 14px', color: '#171717', background: '#d9ad32', border: 0, borderRadius: 8 }} onClick={() => window.location.reload()}>Ricarica vista</button>
      </section>
    </main>;
  }
}
