...
export async function writeAiBundle(project: AbacoProject, parent: string, projectPath?: string, packBlend?: PackBlend, editedMedia?: EditedMedia) {
  ...
  try {
    ...
    const captions = portable.objects.filter(o => o.kind === 'audio' && o.audio.captions.length).map(o => `## ${o.name}\nTempi in secondi relativi alla traccia originale; per il montaggio usare posizione e trim del JSON.\n` + o.audio.captions.map(c => `${c.start.toFixed(3)}–${c.end.toFixed(3)}: ${c.text}`).join('\n'));
    if (captions.length) await fs.writeFile(path.join(stage, 'TRASCRIZIONI.md'), captions.join('\n\n'));
    ...
    const readme = `# Consegna di Scene per l’AI\n\nProgetto: ${portable.name}\n\n1. Leggi ISTRUZIONI_ANIMAZIONE.md${portable.animationStandard ? ' e STANDARD_ANIMAZIONE_ALLEGATO.md' : ''}.\n2. Leggi MEDIA_MONTATI.json: media/AUDIO_MONTATO.wav contiene il montaggio audio già applicato; media/livelli contiene immagini e testi già posizionati e ritagliati, con gli intervalli esatti di utilizzo. Non applicare due volte tagli o trasformazioni. TESTI.json conserva contenuti e animazione del testo.\n3. Apri project.abaco.json: è il progetto completo e autorevole, riapribile in Scene. SCENE.json è un riepilogo.\n4. Risolvi tutti i percorsi relativi dalla cartella di questo documento. assets/ contiene immagini, audio, modelli, anteprime e sfondi effettivamente riferiti. Le anteprime non sostituiscono i modelli originali.\n5. Consulta CONTROLLI_CONSEGNA.json e gli eventuali punti da verificare sotto. MANIFEST.json elenca i file con dimensione e impronta SHA-256.\n6. Importa CAMERE.json con IMPORTA_CAMERE.py in Blender: contiene posizione, orientamento e focale esatti a ogni fotogramma, oltre ai tagli. Rispetta questi dati senza ricomporre le inquadrature.\n7. Costruisci la recitazione sul rig originale, rispettando regia, contatti, camera, fps e sincronizzazioni. I punti del proxy non sono una recitazione finale.\n8. Verifica brevi prove del movimento prima di rifinire e renderizzare tutto. Distingui verifica tecnica, valutazione a velocità reale e approvazione artistica. Non dichiarare svolti controlli che non hai potuto eseguire.\n\nFrequenza: ${portable.settings.fps} fps. Frame: ${portable.settings.frameStart}–${portable.settings.frameEnd}.\n\n${warn.length ? '## Punti da verificare\n\n' + warn.map(w => '- ' + w).join('\n') : 'Tutti i riferimenti diretti agli asset sono stati copiati e verificati.'}\n\nConsegna l’intera cartella mantenendo la struttura. Non occorrono credenziali o impostazioni personali di Scene. Il pacchetto è una consegna di regia, non un video finale già validato.\n`;
