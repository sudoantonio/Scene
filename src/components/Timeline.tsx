...
function subtitleClips(object: SceneObject, range: [number, number], fps: number) {
  ...
  const sourceEnd = trimEnd > trimStart ? trimEnd : duration;
  const sourceLength = Math.max(.01, sourceEnd - trimStart);
  const visibleSeconds = (range[1] - range[0]) / fps;
  const repeats = loop ? Math.ceil(visibleSeconds / sourceLength) : 1;
  return Array.from({ length: repeats }, (_, repeat) => captions.flatMap((caption) => {
    const from = Math.max(trimStart, caption.start);
    const to = Math.min(sourceEnd, caption.end);
    if (to <= from) return [];
...
export default function Timeline({ collapsed, onToggleCollapse }: { collapsed?: boolean; onToggleCollapse?(): void }) {
  ...
  useEffect(() => {
    if (!collapsed) return;
    const overview = collapsedOverviewRef.current;
    if (!overview) return;
    const wheel = (event: WheelEvent) => {
      if (event.ctrlKey || event.metaKey || overview.scrollWidth <= overview.clientWidth) return;
      const delta = Math.abs(event.deltaX) > Math.abs(event.deltaY) ? event.deltaX : event.deltaY;
      const before = overview.scrollLeft;
      overview.scrollLeft += delta;
      if (overview.scrollLeft !== before) event.preventDefault();
    };
    overview.addEventListener('wheel', wheel, { passive: false });
    return () => overview.removeEventListener('wheel', wheel);
  }, [collapsed]);
  useEffect(() => {
    if (!collapsed) return;
    const overview = collapsedOverviewRef.current;
    if (!overview) return;
    const playheadX = (frame - start) / project.settings.fps * timelineZoom;
    if (playheadX < overview.scrollLeft || playheadX > overview.scrollLeft + overview.clientWidth - 12) overview.scrollLeft = Math.max(0, playheadX - overview.clientWidth * .35);
  }, [collapsed, frame, start, project.settings.fps, timelineZoom]);
  ...
  const beginCollapsedPan = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const move = (pointer: PointerEvent) => {
      ...
      if (!moved && Math.abs(pointer.clientX - originX) < 4) return;
      moved = true;
      overview.scrollLeft -= pointer.clientX - previousX;
    ...
    const finish = () => {
      ...
      suppressCollapsedClick.current = moved;
      window.setTimeout(() => { suppressCollapsedClick.current = false; }, 0);
    };
  ...
  const beginMoveMotionPoint = (object: SceneObject, sceneId: string, sceneFrame: number, sceneEnd: number, keyframeId: string, keyframeFrame: number, event: React.PointerEvent<HTMLSpanElement>) => {
    ...
    if (!track) return;
    setPlaying(false);
    select(object.kind === 'camera' ? undefined : object.id);
  ...
  return <section className={`timeline ${collapsed ? 'collapsed' : ''}`}>
    <header className="timeline-toolbar">
      {collapsed && <div className="timeline-mini-row">
        <div className="collapsed-scene-row">
          <div ref={collapsedOverviewRef} className="collapsed-scene-overview" role="group" aria-label="Collapsed scene timeline" onPointerDown={beginCollapsedPan} onClickCapture={(event) => { if (suppressCollapsedClick.current) { event.preventDefault(); event.stopPropagation(); suppressCollapsedClick.current = false; } }}>
    ...
    <div className="timeline-scroll" onClick={seekFromEmptyTimeline} onWheelCapture={zoomTimelineFromWheel} style={{ '--timeline-content-width': `${Math.round(durationSeconds * timelineZoom)}px`, '--timeline-second-width': `${timelineZoom}px`, '--timeline-thumbnail-width': `${Math.max(timelineZoom, 800 / durationSeconds)}px`, '--timeline-playhead-x': `${Math.round(((frame - start) / Math.max(1, end - start)) * Math.max(800, durationSeconds * timelineZoom))}px` } as React.CSSProperties}>
      <div className="ruler-label">Time</div>
      <div className="ruler timeline-ruler" onClick={seek}>{rulerSeconds.map((second) => <span key={second} style={{ left: left(start + second * project.settings.fps) }}>{formatRulerTime(second)}</span>)}</div>
