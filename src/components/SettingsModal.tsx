...
export default function SettingsModal({ onClose }: { onClose(): void }) {
  ...
  return <Modal title="Impostazioni" onClose={onClose}>
    <div className="settings-form">
      ...
      <label className="field"><span>Eseguibile Blender</span><div className="path-picker"><input value={blenderPath} onChange={(event) => setBlenderPath(event.target.value)} placeholder="Vuoto = Blender Flatpak" /><button className="icon" onClick={browse}><FolderOpen size={16} /></button></div><small>Se vuoto usa: flatpak run org.blender.Blender</small></label>
